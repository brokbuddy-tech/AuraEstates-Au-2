...
function SoldPageContent() {
  ...
  const handleSortChange = (value: string) => {
    ...
    if (value === "sold") params.delete("sort"); else params.set("sort", value);
    params.delete("page");
    router.push(`/sold${params.toString() ? `?${params.toString()}` : ""}`);
  };

  const handleLoadMore = () => {
    const params = new URLSearchParams(searchParams.toString());
    params.set("page", String(page + 1));
    router.push(`/sold?${params.toString()}`);
  };
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="flex-1">
              <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                <div>
                  ...
                  <p className="text-[#111111]/40 text-sm font-medium mt-1">
                    Showing {total.toLocaleString()} settled sales{activeQuery ? ` for "${activeQuery}"` : ""}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 text-sm font-bold text-[#111111]/60">
                    ...
                    <Select value={searchParams.get("sort") || "sold"} onValueChange={handleSortChange}>
              ...
              {loading && properties.length === 0 ? (
                ...
                <p className="text-sm uppercase tracking-[0.3em] text-[#111111]/40">No sold listings matched the current filters.</p>
              ) : (
              ...
              {page < totalPages && (
                <div className="mt-20 flex flex-col items-center">
                  <p className="text-black/40 text-sm mb-6">You've viewed {Math.min(page * 12, total)} of {total.toLocaleString()} results</p>
