...
import { getListings, mapListingToAuraProperty, type AuraProperty } from './api';
...
export function replaceTemplateBranding(text: string, agencyName: string) {
  ...
  return text
    ...
    .replace(/\bAura Estates\b/gi, normalizedAgencyName)
    .replace(/AuraEstates/gi, normalizedAgencyName);
}
...
async function buildFallbackAgentProfile(agentSlug: string, agencySlug?: string | null) {
  ...
  return {
    ...
    activeListings,
    soldListings: [] as AuraProperty[],
    rentedListings: [] as AuraProperty[],
  };
}
...
export async function getAgentProfile(agentSlug: string, agencySlug?: string | null) {
  ...
  return {
    ...
    activeListings: Array.isArray(data.activeListings)
      ? data.activeListings.map((listing: any) => mapListingToAuraProperty(listing, agencySlug))
      : [] as AuraProperty[],
    soldListings: Array.isArray(data.soldListings)
      ? data.soldListings.map((listing: any) => mapListingToAuraProperty(listing, agencySlug))
      : [] as AuraProperty[],
    rentedListings: Array.isArray(data.rentedListings)
      ? data.rentedListings.map((listing: any) => mapListingToAuraProperty(listing, agencySlug))
      : [] as AuraProperty[],
  };
}
